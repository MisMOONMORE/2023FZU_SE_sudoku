...
function Sudo(width, height, canvasGame) {
  ...
  this.Init = function (level) {
    ...
    var that = this;
    this.CanvasGame.addEventListener(
      "mouseup",
      function (evt) {
        ...
        if (cell.N == null) {
          if (this.ShowOptionNumber) {
            ...
            for (var k = 0; k < optnList.length; k++) {
              ctx2.fillText(
                optnList[k],
                (j + 0.1) * this.ItemWidth,
                (i + 0.3) * this.ItemHeight + k * (this.ItemHeight / 3),
                this.ItemWidth
              );
            }
            ctx2.restore();
  ...
  this.ProcMouseClick = function (pos) {
    if (this.EditingCellIndex == null) {
      ...
      // 画个版子。
      this.EditPannel = new EditPannel(
        pos.x,
        pos.y,
        this.ItemWidth / 1.5,
        nList,
  ...
  this.CalcOptNumber = function (index) {
    ...
    // 查找横行, 去掉所有已经有的数字
    for (var i = 0; i < 9; i++) {
      if (this.Cells[index.i][i].N == null || i == index.j) continue;

      l[this.Cells[index.i][i].N - 1] = null;
    }
    // 查找竖行, 去掉所有已经有的数字
    for (var i = 0; i < 9; i++) {
      if (this.Cells[i][index.j].N == null || i == index.i) continue;
  ...
  this.CheckConflict = function (index) {
    ...
    // 查找竖行
    for (var i = 0; i < 9; i++) {
      if (this.Cells[i][index.j].N == cell.N && i != index.i) return true;
    }
  ...
  this.ToggleEditMode = function () {
    ...
    if (this.EditMode) {
      ...
      for (var i = 0; i < 9; i++) {
        for (var j = 0; j < 9; j++) {
          if (this.Cells[i][j].N == null) this.Cells[i][j].Fixed = false;
          else this.Cells[i][j].Fixed = true;
        }
      }
    }
...
function EditPannel(x, y, ItemWidth, NList, maxWidth, maxHeight, cv) {
  ...
  if (this.DrawX + this.W > maxWidth) this.DrawX = maxWidth - this.W;
  if (this.DrawY + this.H > maxHeight) this.DrawY = maxHeight - this.H;
}
...
EditPannel.prototype.Draw = function () {
  ...
  for (var i = 1; i <= 9; i++) {
    ...
    this.ctx.strokeStyle = "#000000"; // 设置线的颜色
    this.ctx.strokeRect(itemX, itemY, this.ItemWidth, this.ItemWidth);
    if (InArray(i, this.NList)) {
      this.ctx.fillStyle = "green"; // 设置线的颜色
      this.ctx.font = "Bold " + this.ItemWidth / 2 + "px Arial";
      this.ctx.fillText(
        i.toString(),
        itemX + this.ItemWidth / 3,
        itemY + this.ItemWidth / 1.5
      );
    }
  }
};
